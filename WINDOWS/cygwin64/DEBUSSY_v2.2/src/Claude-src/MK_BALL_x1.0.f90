...
character(10),parameter :: file_inp0 = 'sphmkQ.ini'
...
real(DP)     :: DSPH2MK,DCL2MK(2),minok_xyz
...
call openread_clumkX(clumkfn=file_inp0, &
                     ...
                     min_latokk_xyz=minok_xyz)
if (clushape /= 'SPH') then
  print*,'Shape given is '//clushape//' but I deal only with shape SPH...'
  stop 'I deal only with shape SPH...'
endif
...
print'(a)',' Input file FILE_INP is: '//trim(file_inp)
...
print'(a,g15.8)', ' Working Wavelength  = ',working_wavelength
print'(a,g15.8)', ' 2-Theta Max         = ',working_ttmax
...
if (verbose) print*, 'DCL2MK =', DCL2MK, Delta_R*(NCL2MK+1)*0.2d0
if (verbose) print*, 'Selected Shape = ',clushape
...
!print*,'DVds : '
print*,'  '
print*,'Direct Space Crystal-to-Orthonormal Coordinates Transformation Matrix : '
print'(3(1x,g12.6))',DVds(1,:)
print'(3(1x,g12.6))',DVds(2,:)
print'(3(1x,g12.6))',DVds(3,:)
print*,'  '
DVrs=zero
...
Ncube1 = 1+ceiling((half+real(NCL2MK(1)+1,DP))*Delta_R*LVds)
...
diamax = two*(one+real(NCL2MK(1)+1,DP))*Delta_R  !ten*DCL2MK(1)+two*maxval(eigMT)
...
enddo
call CPU_TIME(ts1)
if (verbose) print*,'Time for radat/mulat: ',ts1-ts0
if (verbose) print*,'NU_CEL_MAX = ',NU_CEL_MAX,ndidi,volu_clus_max/cell_volume,NU_CEL_MAX/(volu_clus_max/cell_volume)
if (verbose) print*,'Allocating check ',ndidi,NU_CEL_MAX,size(Cube1)
...
print*,'Building Spherical Clusters from Shell ',jlower+1,' to Shell ',jupper+1,' by step ',dr_step
print*,'  '
print*,' Diameter (nm)     shell #      shell-max    # of Atoms       CPU Time (sec)    '
print*,'  '

if (output_xyz) call SYSTEM(trim(mkdir_command)//' XYZ'//separator//' > tmp.out 2> tmp.err')

call CPU_TIME(tstart)
t0=tstart
DO ksph=jlower,jupper,dr_step
  radius = real(ksph+1,DP)*Delta_R
  radius2 = radius**2+sceps_DP
  diameter = two*radius
  nlap = (ksph+1)**3
  xlap = real(nlap,DP)
  mmax = min( NCube1, 1+ceiling((half+real(ksph+1,DP))*Delta_R*LVds) )
...
!  where (Cube1Former>yoll) Cube1 = Cube1 - Cube1Former  
  ...
  call CPU_TIME(t1)
  if (verbose) print'(a4,g15.5,2i5,6(1x,g15.5))','*** ',&
                  two*radius/ten,ksph,NCL2MK(1),c000a1,c000b1,xlast,Cube2(0,0,0),t1-t0,t1-tstart
  t0=t1
  mmax2=2*mmax
  do i3=0,mmax2(3)
    aux1(3)=i3
...
!    write(fxyz(lfile_inp-3:lfile_inp+4),'("_",i3.3,".xyz")')ksph
     write(fxyz(lfile_inp-3:lfile_inp+8),'("_",i3.3,"_SPH.xyz")')ksph+1
    iux=find_unit()
    open(iux,status='replace',file='XYZ'//separator//fxyz(1:lfile_inp)//'_SPH_LAT'//fxyz(lfile_inp+1:lfile_inp+8))
    write(iux,*) nlatpoi_clu
    write(iux,*) fxyz(1:lfile_inp-4),ksph
    iuxa=find_unit()
    open(iuxa,status='replace',file='XYZ'//separator//fxyz(1:lfile_inp+8))
    ...
    do i1=-Ncube1(1),Ncube1(1)
      ...
      do i2=-Ncube1(2),Ncube1(2)
        ...
        do i3=-Ncube1(3),Ncube1(3)
          aux1(3)=i3
          ooo=Cube1(i1,i2,i3)
          if (abs(ooo)<=max(minok_xyz,yoll)) cycle
          xyzP=matmul(DVds,aux1)
          if (abs(ooo)>one-yoll) then
            write(iux,'("Si ",3(1x,g14.8),1x,g12.6)')xyzP,one
          else
